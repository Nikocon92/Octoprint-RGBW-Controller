...
class GpiorgbcontrollerPlugin(octoprint.plugin.StartupPlugin,
	...
	def init_rgb(self, red_pin, grn_pin, blu_pin, wht_pin):
		try:
			self.deinit_rgb()
			self.led = RGBLED(red=red_pin, green=grn_pin, blue=blu_pin, white=wht_pin active_high=True, pin_factory=self.pin_factory)
	...
	def on_settings_save(self, data):
		...
		if 'red_pin' in data or 'grn_pin' in data or 'blu_pin' in data or 'wht_pin' in data :
			red_pin = self._settings.get_int(["red_pin"])
			grn_pin = self._settings.get_int(["grn_pin"])
			blu_pin = self._settings.get_int(["blu_pin"])
            wht_pin = self._settings.get_int(["wht_pin"])
			if(red_pin is not None and grn_pin is not None and blu_pin is not None and wht_pin is not None ):
				self.init_rgb(red_pin, grn_pin, blu_pin, wht_pin)
		color = self._settings.get(["color"])
		is_on = self._settings.get_boolean(["is_on"])
		if is_on is not None and color is not None:
			self.color = color
	...
	def replace_color_component(self, color, value, comp):
		if len(color) != 7 or value is None:
			return color
		comp_type = comp.upper()
		if comp_type != "R" and comp_type != "G" and comp_type != "B" and comp_type != "W":
			return color
		if len(value) != 2:
			return color
		old_r = color[1:3]
	...
	def on_gcode_command(self, comm_instance, phase, cmd, cmd_type, gcode, *args, **kwargs):
		...
		if gcode and gcode.startswith("M150"):
            ...
            wht = self.gcode_parse_rgb_component(cmd, "W")
			color = self.replace_color_component(color, wht, "W")
...
__plugin_name__ = "GPIO RGBW Controller"
...
def __plugin_load__():
	...
	__plugin_implementation__ = GpiorgbwcontrollerPlugin()
